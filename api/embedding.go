...
func Embeddings(path string, cfg *config.Config) zoox.HandlerFunc {
	return zoox.WrapH(proxy.New(&proxy.Config{
		OnRequest: func(req, originReq *http.Request) error {
			...
			req.URL.Host = fmt.Sprintf("%s.openai.azure.com", cfg.APIs.Embedding.Resource)
			req.URL.Path = fmt.Sprintf("/openai/deployments/%s%s", cfg.APIs.Embedding.Deployment, path)
			req.Host = req.URL.Host

			originQuery := req.URL.Query()
			originQuery.Set("api-version", cfg.APIs.Embedding.APIVersion)
			...
			req.Header.Set("api-key", cfg.APIs.Embedding.APIKey)
